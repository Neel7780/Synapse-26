...
export default function EventPage() {
  ...
  // Filter fees to only show those with registration open
  const openFees = useMemo(() => {
    return sortedFees.filter((fee) => fee.is_registration_open !== false);
  }, [sortedFees]);

  // Check if all fees are closed (event-level or fee-level)
  const allFeesClosed =
    event?.is_registration_open === false ||
    (sortedFees.length > 0 && openFees.length === 0);
  ...
  if (error || (!loading && !event)) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center font-joker">
        ...
        <Button
          onClick={() => router.back()}
          className="mt-6 rounded-none font-jqka bg-red-600 hover:bg-red-700"
        >
          Go Back
        </Button>
      </div>
  ...
  const currentFee: FormattedFee | null =
    openFees.length > 0 ? openFees[selectedFeeIndex] : null;
  ...
  const eventDateTime = event.event_date
    ? parseDateAsIST(event.event_date)
    : null;
  const eventDate = eventDateTime
    ? eventDateTime.toLocaleDateString("en-IN", {
        day: "numeric",
        month: "short",
        year: "numeric",
  ...
  const eventTime = eventDateTime
    ? eventDateTime.toLocaleTimeString("en-IN", {
        hour: "2-digit",
        minute: "2-digit",
        hour12: true,
        timeZone: "Asia/Kolkata",
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <span>
                Team:{" "}
                {currentFee?.type
                  ? currentFee.type.charAt(0).toUpperCase() +
                    currentFee.type.slice(1)
                  : "Group"}
              </span>
            </div>
          </div>
          ...
          <section className="mt-10">
            ...
            {allFeesClosed ? (
              <div className="flex flex-col gap-6">
                ...
                <Button
                  disabled
                  className="w-full sm:w-fit px-5 py-3 sm:px-8 sm:py-4 lg:px-12 lg:py-6 text-sm sm:text-base lg:text-xl font-jqka tracking-[0.12em] sm:tracking-[0.16em] lg:tracking-[0.2em] uppercase bg-gray-700 border border-gray-600 text-gray-400 rounded-none mt-4 cursor-not-allowed opacity-70 select-none"
                >
                  REGISTRATION CLOSED
                </Button>
              </div>
              ...
              <div className="flex flex-col gap-8">
                <div className="flex flex-wrap items-center gap-4">
                  <span className="text-red-600 text-xl md:text-2xl font-normal">
                    Select Team:
                  </span>
                  ...
                  {openFees.length <= 3 ? (
                    ...
                    <div className="flex bg-white rounded-full relative p-0 overflow-hidden">
                      {openFees.map((fee, index) => {
                        const isActive = selectedFeeIndex === index;
                        return (
                          <button
                            key={fee.fee_id}
                            onClick={() => setSelectedFeeIndex(index)}
                            className={`relative px-6 py-2 text-base font-medium capitalize rounded-full transition-colors duration-300 z-10 ${
                              isActive
                                ? "text-white"
                      ...
                      {openFees.map((fee, index) => {
                        ...
                        return (
                          <button
                            key={fee.fee_id}
                            onClick={() => setSelectedFeeIndex(index)}
                            className={`px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 text-sm sm:text-base font-medium capitalize rounded-full transition-all duration-300 min-w-[70px] sm:min-w-[80px] text-center ${
                              isActive
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    router.push(
                      `/events/${slug}/${eventNameSlug}/register?${queryParams.toString()}`,
                    );
                  }}
                >
                  REGISTER
        ...
        >
          <div className="border border-red-600 rounded-[30px] p-8 md:p-12 relative bg-black shadow-[0_0_20px_rgba(220,38,38,0.1)]">
            ...
            <ul className="space-y-4 mb-10 text-gray-300 text-lg font-light leading-relaxed list-disc pl-5 font-roboto">
              <li>
                Participants must carry their own instruments, props, or tracks
                if required.
              </li>
              <li>
              ...
              <li>
                The judges&apos; and committee&apos;s decision will be final and
                binding.
              </li>
