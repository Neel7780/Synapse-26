...
export default function RegisterPage() {
    ...
    const [teamEmails, setTeamEmails] = useState<string[]>(minMembers > 1 ? [""] : []);
    ...
    const validateTeam = async () => {
        ...
        if (totalMembers < minMembers) {
            setValidationError(`You need at least ${minMembers - 1} more team member(s)`);
            setValidating(false);
            return;
        }
        ...
        try {
            const res = await fetch("/api/events/register", {
                ...
                body: JSON.stringify({
                    ...
                    team_member_emails: feeType !== "solo"
                        ? teamEmails.filter((e) => e.trim())
                        : [],
                    payment_screenshot_url: isDauRegistration ? "DAU_VERIFIED" : paymentScreenshot.trim(),
            ...
            <div className="max-w-2xl mx-auto px-6 py-24">
                ...
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-400">
                        {decodeURIComponent(eventName?.replace(/-/g, " ") || "")} - {feeType}
                    </p>
                ...
                <div className="flex justify-center gap-4 mb-12">
                    <div className={`flex items-center gap-2 ${step === "team" || step === "payment" || step === "success" ? "text-[#b41c32]" : "text-gray-600"}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step === "team" || step === "payment" || step === "success" ? "border-[#b41c32] bg-[#b41c32]/20" : "border-gray-600"}`}>
                            1
                        </div>
                        <span className="hidden sm:inline">Team</span>
                    </div>
                    <div className="w-8 h-px bg-gray-600 self-center" />
                    <div className={`flex items-center gap-2 ${step === "payment" || step === "success" ? "text-[#b41c32]" : "text-gray-600"}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step === "payment" || step === "success" ? "border-[#b41c32] bg-[#b41c32]/20" : "border-gray-600"}`}>
                            2
                        </div>
                        <span className="hidden sm:inline">Payment</span>
                    </div>
                    <div className="w-8 h-px bg-gray-600 self-center" />
                    <div className={`flex items-center gap-2 ${step === "success" ? "text-[#b41c32]" : "text-gray-600"}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step === "success" ? "border-[#b41c32] bg-[#b41c32]/20" : "border-gray-600"}`}>
                            3
                        </div>
                    ...
                    <div className="bg-[#111] border border-white/10 rounded-xl p-8">
                        ...
                        {feeType === "solo" ? (
                            <div className="bg-white/5 border border-white/10 rounded-lg p-4 mb-6">
                                <p className="text-gray-300">Registering as: <span className="text-white font-medium">{user?.email}</span></p>
                            </div>
                        ) : (
                            <>
                                <p className="text-gray-400 mb-4">
                                    You are automatically included as a team member. Add your teammates below.
                                </p>
                                ...
                                <div className="space-y-3 mb-6">
                                    <div className="flex gap-2 items-center">
                                        <div className="flex-1 bg-green-500/10 border border-green-500/30 rounded-md px-3 py-2 text-green-400 flex items-center gap-2">
                                            ...
                                            <span className="text-xs text-green-500/70 ml-auto">(You)</span>
                                        </div>
                                    </div>
                                ...
                                <p className="text-sm text-gray-400 mb-4">
                                    Current team: <span className="text-white font-medium">{1 + teamEmails.filter(e => e.trim()).length}</span> / {maxMembers} members
                                </p>
                        ...
                        <div className="mb-8">
                            {isDauRegistration ? (
                                <div className="text-center p-8 bg-green-500/10 border border-green-500/30 rounded-xl mb-8">
                                    ...
                                    <h3 className="text-2xl font-bold text-green-400 mb-2">DAU Student Verified</h3>
                                ...
                                <>
                                    <p className="text-gray-400 mb-4">
                                        Scan the QR code below and make the payment of <span className="text-[#b41c32] font-bold">₹{feePrice}</span>
                                    </p>
                ...
                {step === "success" && (
                    <div className="bg-[#111] border border-white/10 rounded-xl p-8 text-center">
                        ...
                        <p className="text-sm text-gray-400 mb-8">
                            You will be notified once your payment is verified by the coordinator.
                        </p>
